...
for n=1:N
    ...
    % Thuc hien dich chuyen so nguyen truoc (D(1))
    % Sau do ap dung bo loc cho phan thap phan (D(2))
    
    % Tinh gioi han chi so k cho bo loc
    kmin = max(-p,DeltaStart + n + D(1) - length(Signal));
    kmax = min(p-1,DeltaStart + n + D(1) - 1);
    
    % Kiem tra chi so co hop le khong
    if(DeltaStart + n + D(1) - kmax < 0 || DeltaStart + n + D(1) - kmin > length(Signal))
        Signal_D(n) = 0;
        return
    end
    ...
    Sig = fliplr(Signal(DeltaStart + n + D(1) - kmax : DeltaStart + n + D(1) - kmin));
